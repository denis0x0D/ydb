...
// Ranking reads the row's position, which only the chain can supply, so a window that ranks takes
// the chain whatever its frame says. That is also why a ranking function never rules a window out
// on frame grounds: it does not read the frame at all.
...
bool TPhysicalWindowBuilder::CanBuildWindow(const TOpWindow& window) {
    ...
    for (const auto& func : window.GetWindowFuncs()) {
        ...
        // An aggregate folds row by row, so outside the whole partition path its frame has to run
        // from the partition boundary to the current row.
...
// The fold that produces one state for a whole partition. Fold1 seeds the state from the first row
// and folds the rest, so both lambdas are the same accumulators the chain uses.
...
// A frame spanning the partition gives every row the same value, so the rows are collected once,
// folded to a single state, then read a second time to attach that state. The state is bound as a
// lambda argument rather than referenced inside the map, so the fold runs once.
...
TExprNode::TPtr TPhysicalWindowBuilder::BuildPhysicalOp(TExprNode::TPtr input) {
    ...
    if (Window->GetPartitionKeys().empty()) {
        ...
        auto handler = Ctx.NewLambda(Pos, Ctx.NewArguments(Pos, std::move(handlerArgs)), WholePartition ? BuildWholePartition(flowArg) : BuildChain(flowArg));
