    ...
    if (sublinks.empty()) {
        ...
        for (auto link : sublinks) {
            ...
            if (TKqpExprSublink::Match(link.Get())) {
                entry = TSubplanEntry(subplan.get(), {}, ESubplanType::EXPR, sublinkVar);
            } else if (TKqpExistsSublink::Match(link.Get())) {
                entry = TSubplanEntry(subplan.get(), {}, ESubplanType::EXISTS, sublinkVar);
            } else /* In sublink */ {
                ...
                entry = TSubplanEntry(subplan.get(), {TInfoUnit(TString(tupleElement))}, ESubplanType::IN_SUBPLAN, sublinkVar);
            }
