...
void TOpTableLookup::ComputeOutputIUs() {
    ...
    // In join mode the input rows are passed through and the fetched row is appended to them.
...
TVector<TInfoUnit> TOpTableLookup::GetUsedIUs(TPlanProps& props) {
    Y_UNUSED(props);
    // FetchedRowFilter is deliberately left out: it refers to the fetched columns, which this
    // operator produces rather than reads from its input.
    return LookupKeys;
}
...
void TOpIndexLookupJoin::ComputeOutputIUs() {
    // The table lookup below already produces the joined schema, this operator only reshapes the
