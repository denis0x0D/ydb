...
TStatus ComputeTypes(TIntrusivePtr<TOpTableLookup> lookup, TRBOContext& ctx) {
    ...
    // The filter runs on the fetched row, so it is annotated against the columns of the lookup and
    // not against the columns of its input.
    ...
    // In join mode the stream lookup emits the input row, the row it matched and the number of
    // rows the input row produced. TOpIndexLookupJoin flattens that into a single row.
