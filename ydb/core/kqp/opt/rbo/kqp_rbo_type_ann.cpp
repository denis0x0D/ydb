...
TStatus ComputeTypes(TIntrusivePtr<TOpIndexLookupJoin> lookupJoin, TRBOContext& ctx) {
    ...
    // A semi or an only join outputs the left row alone, the fetched right row only decides whether
    // to emit it.
    if (JoinOutputsRight(lookupJoin->JoinKind)) {
        auto rightItemTypes =
            tupleType->GetItems()[1]->Cast<TOptionalExprType>()->GetItemType()->Cast<TStructExprType>()->GetItems();
