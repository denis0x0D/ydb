...
#define HNDL(name) "KqpLogical-"#name, Hndl(&TKqpLogicalOptTransformer::name)
        AddHandler(0, &TCoEquiJoin::Match, HNDL(EliminateEquiJoinFlatMap));
...
protected:
    ...
    TMaybeNode<TExprBase> EliminateEquiJoinFlatMap(TExprBase node, TExprContext& ctx) {
        TExprBase output = DqEliminateFlatMapBetweenEquiJoins(node, ctx);
        DumpAppliedRule("DqEliminiateFlatMapBetweenEquiJoins", node.Ptr(), output.Ptr(), ctx);
        return output;
    }
